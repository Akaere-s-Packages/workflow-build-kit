# archlinux backend: pacman repo-index-specific publish functions
# (repo-add/repo-remove/vercmp, the <repo>.db.tar.gz/.files.tar.gz layout).
# Sourced, not run standalone — by scripts/publish/minio.sh and
# scripts/publish/publish_all.sh, AFTER they've already sourced
# scripts/publish/repo_lib.sh for the generic S3/mc primitives
# (retry/mc_alias_set/download_metadata_file/upload_package_file/
# upload_public_key) these functions build on. A Debian/apt-style backend
# would replace this whole file with something built around
# apt-ftparchive/reprepro instead — nothing outside backends/archlinux/
# ever calls repo-add/repo-remove/vercmp directly.
#
# Callers must set these globals before calling anything here: repo_name,
# distro, alias_name, remote, db_file, files_file (see minio.sh/
# publish_all.sh for the exact derivation — identical in both).
# GPG_KEY_ID/GPG_PASSPHRASE and the R2_* secrets are read directly from the
# environment. A caller using prune_removed_packages must also pre-declare
# `any_pruned=false` — that function sets it (deliberately not `local`)
# when it actually removes something, the same caller-owned-global
# convention the rest of this file's callers already use.
